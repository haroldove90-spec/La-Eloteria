    ...
    <div className={`w-full transition-all duration-300 ${isFullscreen ? 'fixed inset-0 z-40 bg-elote-cream overflow-y-auto p-4 md:p-8' : 'space-y-6'}`}>
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            <span className="inline-flex items-center gap-1 bg-[#FEF3C7] text-[#92400E] border border-[#FDE68A] text-[10px] px-2 py-0.5 rounded-full font-black uppercase tracking-widest">
              <Coins className="w-3.5 h-3.5" /> Punto de Venta
            </span>
            <span className="text-gray-400 font-bold text-[10px] font-mono tracking-wide uppercase">
              Caja No. 1
            </span>
          </div>
          <h2 className="text-xl md:text-2xl font-black text-[#064E3B] tracking-tight mt-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-8 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {filteredProducts.map(product => (
                <motion.div
                  ...
                  whileTap={{ scale: 0.99 }}
                  className="bg-white border border-[#E5E7EB] rounded-3xl overflow-hidden shadow-2xs hover:shadow-md transition-all flex flex-col justify-between group cursor-pointer"
                ...
                >
                  ...
                  <div className="relative h-44 overflow-hidden bg-gray-100">
                    ...
                    {/* Floating Price tag */}
                    <div className="absolute top-3 right-3 bg-[#064E3B] text-white px-3.5 py-1.5 rounded-2xl font-serif font-black text-sm tracking-tight shadow-md">
                      ${product.price} MXN
                    </div>
                    ...
                    <div className="absolute bottom-3 left-4 right-4">
                      <span className="text-[10px] font-black text-amber-300 font-mono tracking-widest uppercase block mb-0.5">
                        PAQUETE EXCLUSIVO
                      </span>
                      <h3 className="text-white font-serif font-black text-base tracking-wide leading-tight uppercase">
                        {product.name}
                      </h3>
                    </div>
                  </div>
                  ...
                  <div className="p-4 space-y-3 bg-white flex-1 flex flex-col justify-between">
                    ...
                    <button
                      ...
                      className="w-full py-2 bg-[#064E3B] text-white font-serif font-extrabold text-xs uppercase rounded-xl shadow-2xs group-hover:bg-[#065F46] transition-colors flex items-center justify-center gap-1.5 cursor-pointer mt-2"
                    >
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 flex items-center gap-3 text-xs text-amber-900 font-medium">
        ...
        {/* RIGHT COLUMN: Terminal Checkout Drawer */}
        <div className="lg:col-span-4 bg-white border border-[#E5E7EB] rounded-3xl shadow-xs overflow-hidden sticky top-24">
        ...
        <div className="p-5 border-b border-gray-100 bg-gray-50/50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <h3 className="text-base font-serif font-black text-[#064E3B]">
              Historial del Turno de Ventas (Auditoría de Caja)
            </h3>
          ...
          <div className="bg-emerald-50 px-3 py-1.5 rounded-xl border border-emerald-100 text-[#064E3B] flex items-center gap-1.5 self-stretch sm:self-auto text-center justify-center">
            ...
            <span className="text-[10px] font-black font-mono uppercase tracking-wider">
              Total Turno: ${salesHistory.reduce((s, x) => s + x.total, 0)} MXN
            </span>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className="bg-gray-100 text-[#064E3B] font-mono text-[9px] font-bold uppercase tracking-wider border-b border-gray-200">
                <th className="py-3 px-5">No. Ticket</th>
                <th className="py-3 px-5">Fecha / Hora</th>
                <th className="py-3 px-5">Cliente</th>
                <th className="py-3 px-5">Productos Vendidos</th>
                <th className="py-3 px-5">Método Pago</th>
                <th className="py-3 px-5 text-right">Total Cobrado</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {salesHistory.map((tx) => (
                <tr key={tx.id} className="hover:bg-amber-500/5 transition-colors">
                  <td className="py-3.5 px-5 font-mono font-bold text-[#064E3B]">{tx.ticketNumber}</td>
                  <td className="py-3.5 px-5 text-gray-500 font-mono">{tx.timestamp}</td>
                  <td className="py-3.5 px-5 font-bold text-gray-800">{tx.customerName}</td>
                  ...
                  <td className="py-3.5 px-5 text-right font-mono font-bold text-amber-600">${tx.total} MXN</td>
